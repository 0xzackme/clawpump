...
async function realCreateToken(opts) {
    ...
    if (opts.agentWallet) {
        console.log('[REAL] Setting up fee sharing (70/30) in single transaction...');
        ...
        // Update fee shares: 7000 bps (70%) agent, 3000 bps (30%) platform
        const updateSharesIx = await sdk.updateFeeShares({
            ...
            newShareholders: [
                { address: agentPubkey, shareBps: 7000 },      // Agent: 70%
                { address: platformKeypair.publicKey, shareBps: 3000 }, // Platform: 30%
            ],
        });
        ...
        console.log('[REAL] Agent:', agentPubkey.toBase58(), '(70%)');
        console.log('[REAL] Platform:', platformKeypair.publicKey.toBase58(), '(30%)');
    }
