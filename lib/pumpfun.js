 ...
 * @param {string} [opts.agentWallet] Agent's Solana wallet — for 65/35 fee split
...
async function realCreateToken(opts) {
    ...
    // Fee sharing will be set up separately to split 65/35
    ...
    // Step 5: Set up fee sharing (65% agent / 35% platform)
    if (opts.agentWallet) {
        console.log('[REAL] Setting up fee sharing (65/35) in single transaction...');
        ...
        const createConfigIx = await sdk.createFeeSharingConfig({
            ...
            mint: mintKeypair.publicKey,
            pool: null, // null for ungraduated coins
        });

        // Update fee shares: 6500 bps (65%) agent, 3500 bps (35%) platform
        const updateSharesIx = await sdk.updateFeeShares({
            authority: platformKeypair.publicKey,
            mint: mintKeypair.publicKey,
            currentShareholders: [platformKeypair.publicKey], // Platform is current creator
            newShareholders: [
                { address: agentPubkey, shareBps: 6500 },      // Agent: 65%
                { address: platformKeypair.publicKey, shareBps: 3500 }, // Platform: 35%
            ],
        });
        ...
        console.log('[REAL] Fee sharing configured (single tx)! Tx:', feeTxSig);
        console.log('[REAL] Agent:', agentPubkey.toBase58(), '(65%)');
        console.log('[REAL] Platform:', platformKeypair.publicKey.toBase58(), '(35%)');
    }
