...
/**
 * Parse a post body for !ClawdPump launch details.
 * 
 * Supports:
 ...
 */
export function parseClawdPumpPost(content) {
    if (!content || typeof content !== 'string') {
        return { success: false, error: 'Empty or invalid content' };
    }

    // Must contain trigger
    if (!content.toLowerCase().includes('!clawdpump')) {
        return { success: false, error: 'Missing !ClawdPump trigger' };
    }
...
function tryParseKeyValue(content) {
    ...
    for (const line of lines) {
        ...
        if (!trimmed || trimmed.toLowerCase().startsWith('!clawdpump')) continue;
