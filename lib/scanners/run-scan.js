...
export async function runScan({ platform, fetchPosts, extractPostData, reply }) {
    ...
    for (const rawPost of posts) {
        ...
        const { canLaunch, reason, balance } = await canAgentLaunch(launchWallet);
        if (!canLaunch) {
            await markPostProcessed({ id: postId, platform, agentId: agent.agentId, status: 'insufficient_balance', errorMsg: 'Insufficient $CLAWDPUMP' });
            try {
                await reply(postId, `❌ **Insufficient $CLAWDPUMP Balance**\n\nTo launch tokens for free, you need to hold at least **2,000,000 $CLAWDPUMP** in your wallet.\n\n**Your balance:** ${balance.toLocaleString()} $CLAWDPUMP\n**Required:** 2,000,000 $CLAWDPUMP\n\nBuy $CLAWDPUMP: https://pump.fun/coin/4jH8AzNS9op6fKNNzxmmagvqpbC2egwHRxBsaUjDfQLk`);
            } catch { /* best effort */ }
            continue;
        }

        // Rate limit: 6 per 24h per agent (shared across all platforms)
        ...
        if (launchCount >= 6) {
            ...
            try {
                await reply(postId, `⏳ **Rate limited**\n\n6 launches per 24 hours per agent.\nYou've used ${launchCount}/6 in the current window.`);
            } catch { /* best effort */ }
            continue;
        }
